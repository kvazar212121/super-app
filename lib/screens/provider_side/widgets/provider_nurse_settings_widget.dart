...
/// Hamshira — uyga chiqish, tibbiy xizmatlar, vaqt.
class ProviderNurseSettingsWidget extends StatefulWidget {
  final String categoryKey;
  final Color accent;

  const ProviderNurseSettingsWidget({
    super.key,
  ...
  @override
  State<ProviderNurseSettingsWidget> createState() =>
      _ProviderNurseSettingsWidgetState();
}

class _ProviderNurseSettingsWidgetState extends State<ProviderNurseSettingsWidget> {
  ...
  final Set<String> _medicalTypes = {};
  ...
  void _applyMeta(Map<String, dynamic> meta) {
    ...
    _medicalTypes
      ..clear()
      ..addAll(
        (meta['medical_types'] as List<dynamic>? ?? ['injection', 'blood_test'])
            .map((e) => e.toString()),
      );
    _areaCtrl.text = meta['service_area']?.toString() ?? '';
    _qualCtrl.text = meta['qualifications']?.toString() ?? '';
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final meta = Map<String, dynamic>.from(_baseMeta)
        ...
        ..['medical_types'] = _medicalTypes.toList()
        ..['services'] = services
        ..['prices'] = prices
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Text(
          'Hamshira xizmatlari (uyga chiqish)',
        ...
        TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'Malaka / litsenziya',
            border: OutlineInputBorder(),
          ),
        ),
        const SizedBox(height: 16),
        Text('Tibbiy xizmatlar', style: Theme.of(context).textTheme.titleMedium),
